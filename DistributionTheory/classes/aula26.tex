...
\end{crl*}
\begin{proof*}
	Antes de tudo, sendo \(f\in \mathcal{C}^{\infty}\) com \(\partial ^{\alpha }f\in \mathcal{C}_{0}\), para todo \(\alpha \), tem-se
	\[
		(\partial _{x}^{\alpha }f)^{\widehat{\; }}=(2\pi i\xi )^{\alpha }\hat{f}
	\]
	e \(x^{\beta }f\in L^{1}\), para todo \(\beta \in \mathbb{Z}_{+}^{n}\), ditam que \(\hat{f}\in \mathcal{C}^{\infty}\) e
	\[
	...
	\begin{align*}
		\partial _{\xi }^{\beta }\bigl(\xi ^{\alpha }\hat{f}\bigr)
		 ...
		 & =(-1)^{|\beta |}(2\pi i)^{|\beta |-|\alpha |}\bigl[x^{\beta }(\partial _{x}^{\alpha }f)\bigr]^{\widehat{\; }}. \text{ \qedsymbol}
	\end{align*}
\end{proof*}
...
Duas das principais ferramentas (ou operações) de trabalho da Análise de Fourier são a ``convolução'' e a ``Transformada de Fourier'', sendo essa última a definitiva para a consolidação da teoria das Distribuições (em detrimento de outras baseadas em medidas e técnicas parecidas).

A transformada é, na análise de Fourier, estudada em dois contextos (ou níveis):
\begin{itemize}
...
Outro elemento importante é que os ``núcleos'' que produzem as soluções não têm a mesma forma:
\begin{itemize}
	\item[(i)] tem
	      \[
...
\begin{example}
	...
	\begin{tcolorbox}[
		...
		Escolhendo valores específicos de \(a\):
		\[
		...
		Observe que O cálculo realizado em (ii), por si só, é interessante, pois, tomando \(g_{a}=\chi _{(0,\infty )}f_{a}\), vimos que
		\[
...
\begin{example}
	\textbf{(3) \underline{IMPORTANTÍSSIMO:}} Agora em \(\mathbb{R}^{n}\). Seja ainda \(a>0\) e consideremos \(f:\mathbb{R}^{n}\rightarrow \mathbb{R}\) dada por
	\[
		f(x)=e^{-a\pi |x|^{2}}.
	\]
	...
	\]
	e existe \(c\in \mathbb{C}\) tal que
	\[
	...
	\begin{tcolorbox}[
		...
		Por outro lado, tomando \(a=4\pi t\), \(t>0\), obtemos
		\[
...
Queremos agora estudar de perto o comportamento da \(\mathcal{F}\) nas funções de \(\mathcal{S}\) (o espaço de Schwartz: funções de decaimento rápido). O primeiro passo consiste em estabelecer uma (ou outra) caracterização das funções de \(\mathcal{S}\).
...
\begin{proof*}
	...
	\]
	sendo \(\alpha =Ne_{j}=(0,\dotsc ,0,N,0,\dotsc ,0)\). Com isso, concluímos que, para todos \(N\) e \(\beta \in \mathbb{Z}_{+}^{n}\),
	\begin{align*}
	...
	\]
	ou é nula (se algum \(\gamma _{j}>\alpha _{j}\)), elas são da forma ``\(x^{\alpha }\)'', donde temos
	\[
		\partial ^{\beta }(x^{\alpha }f)
	...
	\]
	implique \(P_{(\eta ,\delta )}(f)<\infty \) quando \(|\eta |\leq m\) e \(|\delta |\leq k\), para certos \(m,k\in \mathbb{N}\), e mostremos que isso também ocorre quando \(|\eta |=m+1\) ou \(|\delta |=k+1\). Note que
	\[
...
\begin{proof*}
	...
	\]
	conforme a segunda observação acima e o lema. Dessa forma, as fórmulas (VI) e (VII) podem ser aplicadas em conjunto para escrevermos, para \(\alpha ,\beta \in \mathbb{Z}_{+}^{n}\) arbitrários,
	\begin{align*}
		\partial _{\xi }^{\beta }\bigl(\xi ^{\alpha }\hat{f}\bigr)
		 & =\partial _{\xi }^{\beta }\bigl[(2\pi i)^{-|\alpha |}((2\pi i\xi )^{\alpha }\hat{f})\bigr]                     \\
		 & =(2\pi i)^{-|\alpha |}\partial _{\xi }^{\beta }\bigl[(2\pi i\xi )^{\alpha }\hat{f}\bigr]                       \\
