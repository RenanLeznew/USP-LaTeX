...
\subsection{Soluções Fundamentais e Densidade das Funções Teste em D}
\begin{def*}
...
\begin{example}
	...
	\end{align*}
	ou seja, \(P(D)u = f\) em \(\mathcal{D}'.\) Aqui, utilizamos
	\[
...
\begin{proof*}
	...
	\[
		\langle (\psi u)*\varphi_{\varepsilon }, \phi \rangle = \langle \psi u, [\varphi_{\varepsilon }*(\check{\varphi })\check{]} \rangle\stackrel{\varepsilon \to 0^{+}}\rightarrow  \langle \psi u, (\check{\varphi }\check{)}  \rangle = \langle \psi u, \varphi  \rangle.
	\]

	Dada \(\varphi \in \mathcal{C}_{c}^{\infty}\), para j natural e \(\varepsilon > 0\), podemos escrever
	\begin{align*}
		\langle (\psi_{j}u)*\varphi_{\varepsilon }, \varphi  \rangle & = \langle (\psi_{j}u)*\varphi_{\varepsilon }-\psi_{j}u, \varphi  \rangle+\langle \psi_{j}u, \varphi  \rangle                                    \\
		                                                             & = \langle (\psi_{j}u)*\varphi_{\varepsilon }-\psi_{j}u, \varphi  \rangle + \langle \psi_{j}u-u, \varphi  \rangle + \langle u, \varphi  \rangle.
	\end{align*}
	Dado \(\eta > 0\) e fixando \(j_{0}\) tal que a segunda parcela é menor que \(\eta/2\), utilizando este \(j_{0}\), podemos fazer \(\varepsilon > 0\) ficar suficientemente pequeno para que a primeira parcela também seja menor que \(\eta/2\), finalizando quase que de vez o teorema.
	Dito isso, dada \(\varphi \in \mathcal{C}_{c}^{\infty},\) calculamos:
	\begin{align*}
		\langle (\psi u)*\varphi_{\varepsilon }, \varphi  \rangle = \langle (\psi u)*\varphi_{\varepsilon }, \tau_{0}\varphi  \rangle & = \langle (\varphi u)*\varphi_{\varepsilon }, \tau_{0}[\check{\varphi }\check{]} \rangle \\
		                                                                                                                              & = [(\psi u)*\varphi_{\varepsilon }]*(\check{\varphi })(0)                                \\
		                                                                                                                              & = (\psi u)*[\varphi_{\varepsilon }*(\check{\varphi })](0)                                \\
		                                                                                                                              & = \langle \psi u, \tau_{0}(\varphi_{\varepsilon }*(\check{\varphi })\check{)} \rangle    \\
		                                                                                                                              & = \langle \psi u, [\varphi_{\varepsilon }*(\check{\varphi})\check{]\;\;}\rangle          \\
		                                                                                                                              & = \langle \psi u, [\varphi_{\varepsilon }*(\check{\varphi})\check{]\;\;} \rangle.
	\end{align*}
	Assim, como \(\varphi_{\varepsilon }*(\check{\varphi })\) converge para \(\check{\varphi }\) em \(\mathcal{C}_{c}^{\infty}\) quando \(\varepsilon \) tende a 0 e \(R:\mathcal{C}_{c}^{\infty}\rightarrow \mathcal{C}_{c}^{\infty}\) dada por \(R\varphi =\check{\varphi }\) é contínua, vem da última igualdade o que precisávamos provar:
	\[
		\langle (\psi u)*\varphi_{\varepsilon }, \varphi  \rangle=\langle \psi u, [\varphi_{\varepsilon }*(\check{\varphi })\check{]\;\;} \rangle\stackrel{\varepsilon \to0^{+}}\longrightarrow \langle \psi u, (\check{\varphi }\check{)\;} \rangle=\langle \psi u, \varphi  \rangle. \text{ \qedsymbol}
	\]
\end{proof*}
