...
\]
Assim, para a base de diferenciais, a ação delas em um elemento \((v_1,\dotsc , v_{k})\) é
\[
	\mathrm{d}x_{i_1}\wedge \dotsc \wedge \mathrm{d}x_{i_{k}} = \det{(v_{i_k})},
\]
...
\]
Em particular, se g é uma 0-forma, então \(f^{*}(g) = g\circ f\), ou seja, a \(f^{*}\) equivale a um tipo generalizado de substituição de variáveis. Definimos, assim,

\begin{def*}
	O \textbf{pullback} de uma k-forma \(\omega \) é uma função que leva k-formas de \(\mathbb{R}^{m}\) em k-formas de \(\mathbb{R}^{n}\), \(f^{*}:\Lambda^{k}(\mathbb{R}^{m})\rightarrow \Lambda^{k}(\mathbb{R}^{n})\), induzida por uma função \(f:\mathbb{R}^{n}\rightarrow \mathbb{R}^{m}\), definido por
	\[
		(f^{*}\omega )(p)(v_1, \dotsc , v_{k}) = \omega(f(p))(df_p (v_1), \dotsc , df_p(v_k)).\; \square
	\]
\end{def*}
...
\begin{tcolorbox}[
	...
	Quando um pullback \(f^{*}\) age sobre uma k-forma do tipo
	\[
		\omega  = \sum\limits_{I}^{}a_{I}\mathrm{d}x_{I},
	\]
	os diferenciais mudam de acordo com \textit{projeções} (afinal, \(\mathrm{d}x_{I}\) denota o diferencial da projeção! Lembre-se da aula de introdução deles). Assim,
	\[
		f^{*}\omega (p) = \sum\limits_{I}^{}a_{I}(f(p))\mathrm{d}(x_{I} \circ f)_{p},
	\]
	onde \(x_{I} \circ f\) denota as projeções. Por exemplo, se \(f(p) = (f_1(p), f_2(p))\), então
	\[
		x_{1}\circ f = f_1(p) \quad\&\quad x_{2}\circ f = f_2(p)
	\]
\end{tcolorbox}
\begin{prop*}
	...
	\begin{align*}
		 ...
		 & \text{c) } f^{*}(\varphi_1 \wedge \dotsc \wedge \varphi_{k}) = f^{*}\varphi_1 \wedge \dotsc \wedge f^{*}\varphi_{k}.
	\end{align*}
\end{prop*}
\begin{example}
	Considere a 1-forma em \(\mathbb{R}^{2}\setminus{\{(0, 0)\}}\) dada por
	\[
...
\begin{prop*}
	...
	\begin{align*}
		 & \text{a) }f^{*}(\omega \wedge \varphi ) = (f^{*}\omega  )\wedge (f^{*}\varphi ); \text{ e} \\
