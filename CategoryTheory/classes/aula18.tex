...
\begin{example}
	...
	\begin{center}
		\begin{tikzpicture}[
			...
			\draw[Arrow](TL)to[out = 250, in = 150, edge node={node[midway, left] {\(\beta_{m}\)}}](BR); % To use in/out, imagine a circle around the node. The angles are with respect to the node as the center.
...
\begin{example}
	...
	\begin{center}
		\begin{tikzpicture}[
			...
			\draw[Arrow, scale=.7](TL)to[out = 250, in = 150, edge node={node[midway, left] {\(\beta_{x}\)}}](BR); % To use in/out, imagine a circle around the node. The angles are with respect to the node as the center.
			\draw[Arrow](TR)to[out = 300, in = 30, edge node={node[midway, right] {\(\beta_{x}\)}}](BR);
		\end{tikzpicture}
	\end{center}
	which implies
	\[
