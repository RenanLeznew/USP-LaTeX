...
\begin{proof*}
	...
	\begin{center}
		\begin{tikzpicture}[
			...
			\draw[Arrow, dashed](BL)--node[midway, right] {\(\lambda \)}(BR);
			\draw[Arrow](TL)--node[midway, right] {\(\alpha_{j}\)}(BL);
			\draw[Arrow](TR)--node[midway, left] {\(\alpha_{j'}\)}(BL);
			\draw[Arrow, -stealth](TL)to[out = 250, in = 150, edge node={node[midway, left] {\(\alpha'_{j}\)}}](BR); % To use in/out, imagine a circle around the node. The angles are with respect to the node as the center.
			\draw[Arrow, -stealth](TR)to[out = 300, in = 30, edge node={node[midway, right] {\(\alpha'_{j'}\)}}](BR);
		\end{tikzpicture}
	\end{center}
