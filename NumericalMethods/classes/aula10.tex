...
\[
	\overline{x} = \pm M \times B^{E} = \pm (0, d_1 d_2, \dotsc , d_{t})_{\beta }\times \beta^{E}.
\]
...
Com a notação dos pontos flutuantes, um sistema de mantissa dessa forma pode ser escrito como \(\mathbb{F}(\beta , t, m, M)\), com \([m, M]\) sendo o intervalo do expoente.
\begin{example}
	No sistema \(\mathbb{F}(2, 3, -2, 1)\), a quantidade de números não nulos que podem ser representados é \(2 \times 1 \times 2 \times 2 \times 4 = 32\) números; no sistema \(\mathbb{F}(10, 3, -5, 5)\), o menor e maior número em valor absoluto que podem ser representados é \(l = 10^{-6}\) e \(u = 99900.\)
\end{example}
...
Com base no que foi visto, os números \(\overline{x}\in \mathbb{F}\) são limitados às cotas
\[
	l \leq | \overline{x} | \leq u,
\]
tal que, se \(| \overline{x} | < l\), o erro é chamado de \textbf{underflow}, e, se \(| \overline{x} | > u\) é chamado de \textbf{overflow}.
\begin{listing}[H]
