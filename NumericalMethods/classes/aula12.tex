...
\begin{document}
\section{Aula 12 - 14 de Maio, 2026}
...
Tendo uma fórmula iterativa como esta, podemos implementar o método Cholesky em Python:
\begin{listing}[H]
	\caption{Método de Cholesky em Python}
	\begin{minted}[bgcolor = DarkBackground, linenos=true, breaklines]{Python}
import numpy as np

def cholesky_simples(A):
    # Garantimos que A é um array NumPy do tipo float
    ...
    for i in range(n):
        for j in range(i + 1):
            # O produto escalar substitui o loop 'sum(...)' instantaneamente
            tmp_sum = L[i, :j] @ L[j, :j]
...
# Testando
A = [[3, 2, 0], [2, 5, 1], [0, 1, 3]]
L = cholesky_simples(A)

print("Matriz L (Vetorizada):")
print(L)
\end{minted}
\end{listing}
Curiosamente, o próprio scipy já tem este método implementado, então dá pra poupar esforços futuros:
\begin{listing}[H]
	\begin{minted}[bgcolor = DarkBackground, linenos=true, breaklines]{Python}
import numpy as np

A = np.array([[3, 2, 0], [2, 5, 1], [0, 1, 3]], dtype=float)

# Por padrão, o NumPy retorna a matriz triangular inferior (L)
L_np = np.linalg.cholesky(A)

print("Matriz L (NumPy nativo):")
print(L_np)
\end{minted}
\end{listing}
