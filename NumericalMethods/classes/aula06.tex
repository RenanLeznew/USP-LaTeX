...
maskAc = ((Ac > 7) & (Ac < 15))
Ac[maskAc] = -1
print('\nAc modificada com -1 para elementos maiores que 7 e 
menores que 15')
print(Ac)
\end{minted}
...
\begin{minted}[bgcolor=DarkBackground, linenos=true]{Python}
mask = [(i%2==0) or (j%2==0) for i in range(A.shape[0]) for j 
in range(A.shape[1])]
...
mask = [True if i>j else False for i in range(A.shape[0]) for j in
range(A.shape[1])]
mask = np.array(mask).reshape(A.shape[0],A.shape[1])
print(mask)
