...
\begin{example}[Encaixando Euler]
	...
	\[
		\underline{b}^{n}(\Delta t) = \Delta t \underline{g}^{n}, \quad B(\Delta t) = (I+\Delta tA).
	\]
\end{example}
...
\[
	\boxed{U_{j}^{n+1} = g(\zeta )e^{ijh\zeta }}
\]
onde \(g(\zeta )\) é o \textbf{fator de amplificação} para o número de onda \(\zeta \). Substituindo estes termos no método, obtemos
\[
...
\begin{exr}
	...
	\[
		U_{j}^{n+1} = U_{j}^{n} + \frac{\Delta t}{2h^{2}} (U_{j-1}^{n} - 2U_{j}^{n} + U_{j+1}^{n} + U_{j-1}^{n+1} - 2U_{j}^{n+1} + U_{j+1}^{n+1}).
	\]
