...
\[
	\mathcal{D}_{E}(u(x_{j}, t_{n})) = \{x_{j}-at_{n}\} \quad\&\quad \mathcal{D}_{N}(U_{j}^{n}) = [x_{j}-n\Delta x, x_{j}],
\]
tal que a \hyperlink{cfl_condition}{\textit{Condição CFL}} passa a ser
\[
	\mathcal{D}_{E}\subseteq \mathcal{D}_{N} \Longleftrightarrow x_{j}-nh \leq x_{j}-an\Delta t\leq x_{j} \Longleftrightarrow -n\Delta x \leq -an\Delta t\leq 0,
\]
...
Logo, mesmo com condições iniciais suaves, os problemas não lineares ainda ficam descontínuo, e o que significa uma solução descontínua de uma equação que tem derivadas?
