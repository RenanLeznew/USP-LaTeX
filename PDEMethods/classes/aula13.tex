...
\end{itemize}
\subsection{Equação do Calor: método progressivo, retrógrado e Crank-Nicolson.}
...
Veremos mais alguns métodos hoje, incluindo o método regressivo e o método de \textit{Crank-Nicolson}
...
Quanto ao método de Crank-Nicolson, ele seria um método de diferenças centrais com uma maracutaia -- se fizermos um método por diferenças centrais usuais, gostaríamos de evitar fazer dois níveis por enquanto por conta da quantidade de pontos que ele vai envolver; faremos uma ampliação da malha espaço-temporal com \(t_{n+1/2} = t_{n} + \frac{\Delta t}{2}\), dando
\begin{align*}
...
\begin{align*}
	...
	\Rightarrow & -\frac{\sigma }{2}U_{j+1}^{n+1} + (1+\sigma )U_{j}^{n+1} -\frac{\sigma }{2}U_{j+1}^{n+1} = \frac{\sigma }{2}U_{j-1}^{n}+(1-\sigma )U_{j}^{n}+\frac{\sigma }{2}U_{j+1}^{n},
\end{align*}
cuja matriz fica
\begin{align*}
	  & \begin{bmatrix}
...
\]
resultando no método de Crank-Nicolson
\[
	\boxed{-\frac{\sigma }{2}U_{j+1}^{n+1} + (1+\sigma )U_{j}^{n+1} -\frac{\sigma }{2}U_{j+1}^{n+1} = \frac{\sigma }{2}U_{j-1}^{n}+(1-\sigma )U_{j}^{n}+\frac{\sigma }{2}U_{j+1}^{n}}
\]
