...
\begin{tcolorbox}[
	...
	Alguns exemplos de \hypertarget{boundary_conditions}{condições de contorno} com \(\Omega = [0, L]\) e que garantem o requerimento acima são:
	\begin{align*}
		 & u(0) = u(L) = v(0) = v(L) = 0 \text{ (Dirichlet)}                                                  \\
		 & u'(0) = u'(L) = v'(0) = v'(L) = 0 \text{ (Neumann)}                                                \\
		 & u(x, y)+\beta (\nabla u(x, y)\cdot n(x, y)) = g(x, y),\; (x, y)\in \partial \Omega \text{ (Robin)} \\
...
\]
Podemos discretizar esta equação utilizando a aproximação por diferenças centradas, colocando \(a_{j}\coloneqq a(x_{j}),\; b_{j}\coloneqq b(x_{j}),\; c_{j}\coloneqq c(x_{j})\) e \(f_{j} \coloneqq f(x_{j})\):
\[
	a_{j}\frac{U_{j-1}-2U_{j}+U_{j+1}}{h^{2}} + b_{j}\frac{U_{j+1}-U_{j-1}}{2h}+c_{j}U_{j}=f_{j},
\]
colocando \(U_{0}=\alpha \) e \(U_{m+1}=\beta \). Como consequência, obtemos uma equação da forma \(AU=F\), mas onde, dessa vez,
\[
	A = \frac{1}{h^{2}}\begin{bmatrix}
		(h^{2}c_{1}-2a_{1})      & (a_1+\frac{hb_1}{2}) & \dotsc                    &                     \\
		(a_{2}-\frac{hb_{2}}{2}) & (h^{2}c_{2}-2a_{2})  & (a_{2}+\frac{hb_2}{2})    & \dotsc              \\
		\vdots                   & \ddots               & \ddots                    & \dotsc              \\
		                         &                      & (a_{m}- \frac{hb_{m}}{2}) & (h^{2}c_{m}-2a_{m})
	\end{bmatrix}
\]
e
\[
	U = \begin{bmatrix}
		...
		U_{m}
	\end{bmatrix},\; F = \begin{bmatrix}
		f(x_1)-\alpha \bigl(\frac{a_1}{h^{2}} - \frac{b_1}{2h}\bigr) \\
		f(x_2)                                                       \\
		\vdots                                                       \\
		f(x_m)-\beta \bigl(\frac{a_m}{h^{2}} - \frac{b_m}{2h}\bigr)
	\end{bmatrix}.
\]
Vale mencionar que essa discretização pode não ser a melhor para certos tipos de problemas físicos, mesmo quando há acurácia de segunda ordem; muitas vezes, os problemas físicos possuirão certas propriedades que gostaríamos de preservar quando fazemos a discretização, e é importante entender o problema sendo modelado, além de ter sempre em mente as propriedades matemáticas dele antes de sair aplicando um método numérico.
Por exemplo, considere a condução de calor em um barra com propriedades de condução que variam dependendo do pedaço considerado, ou seja, \(\kappa (x)\) varia com cada x e \(\kappa (x)>0\) para todo ponto x entre os extremos a e b da barra, resultando na equação
\[
...
\]
a partir da qual podemos aplicar a discretização; porém, essa abordagem não é a melhor! Afinal, o sistema linear resultante pode não ser simétrico!!
O melhor seria discretizar o problema física diretamente
\[
	\kappa (x_{j+1/2})u'(x_{j+1/2})=\kappa_{j+1/2}\biggl(\frac{U_{j+1}-U_{j}}{h}\biggr),
\]
junto à aproximação em \(x_{j-1/2}\) de maneira análoga; assim, fazendo a diferença entre eles, obtemos uma aproximação centrada em \(x_{j}\) para \((\kappa u')'\):
\begin{align*}
	(\kappa u')' & \approx \frac{1}{h}\biggl[\kappa (x_{j+1/2})u'(x_{j+1/2}) - \kappa (x_{j-1/2})u'(x_{j-1/2})\biggr]                                        \\
	             & \approx \frac{1}{h}\biggl[\kappa _{j+1/2}\biggl(\frac{U_{j+1}-U_{j}}{h}\biggr)-\kappa_{j-1/2}\biggl(\frac{U_{j}-U_{j-1}}{h}\biggr)\biggr] \\
	             & = \frac{1}{h^{2}}(\kappa_{j-1/2}U_{j-1}-(\kappa_{j-1/2}+\kappa_{j+1/2})U_{j}+\kappa_{j+1/2}U_{j+1}),
\end{align*}
que leva à matriz simétrica
\[
	A=\frac{1}{h^{2}}\begin{bmatrix}
		-(\kappa_{1/2}+\kappa_{3/2}) & \kappa_{3/2}                 & \dotsc                             &                                  \\
		\kappa_{3/2}                 & -(\kappa_{3/2}+\kappa_{5/2}) & \kappa_{5/2}                       & \dotsc                           \\
		\vdots                       & \ddots                       & \ddots                             & \dotsc                           \\
		                             & \kappa_{m-3/2}               & -(\kappa_{m-3/2} + \kappa_{m-1/2}) & \kappa_{m-1/2}                   \\
		                             &                              & \kappa_{m-1/2}                     & -(\kappa_{m-1/2}+\kappa_{m+1/2})
	\end{bmatrix}
\]
A grande vantagem da matriz fornecida por esse método é que ela é simétrica, afinal a equação diferencial que dá origem a ela é autoadjunta; além disso, como \(\kappa >0\), a matriz obtida será não sigular e definida negativa!
