...
O terceiro método, das \textit{correções atrasadas/defasadas}, tem uma vantagem de resolver os dois problemas na mesma malha ao invés de refinar ela como no anterior. Para entender como ele funciona, vamos considerar um exemplo de resolução do sistema
\[
...
\]
com termos de fronteira adicionados nos pontos \(j=1\) e \(j=m\); daí, basta resolver \(AU=F\) para obter uma solução de acurácia de quarta ordem diretamente. Resumidamente: utilizamos a solução aproximada para estimar o erro de truncamento local, depois resolvemos um problema auxiliar para sair do erro local para o global, e com ele, pudemos melhorar a solução aproximada! Not eque
\[
	\lim_{h\to 0}F_{j} = f(x),
\]
de modo que
\[
	\frac{1}{h^{2}}\biggl\{U_{j-1}-2U_{j}+U_{j+1}\biggr\} = F_{j}
\]
resulta numa solução consistente com \(u''(x)=f(x)\) e de quarta ordem.
De fato, basta observar o erro de truncamento local do método:
\begin{align*}
	\tau_{j} & =\frac{1}{h^{2}}\biggl\{u(x_{j-1}-2u(x_{j})+u(x_{j+1}))\biggr\}-g(x_{j})                      \\
	         & =u''(x_{j})+\frac{h^{2}}{12}u^{(4)}(x)+\mathcal{O}(h^{4})-f(x_{j})-\frac{h^{2}}{12}f''(x_{j}) \\
	         & = \mathcal{O}(h^{4}),
\end{align*}
já que \(u''=f\) e \(u^{(4)}=f''\).
...
\]
novamente a menos de condições de contorno. Podemos simplificar ainda mais supondo que o coeficiente \(\kappa \) é constante em \(\Omega \) e escrevendo
\[
	f= -\frac{\psi }{\alpha },\quad \kappa (x, y)\equiv c > 0,\; (x,y)\in \Omega .
\]
A terminologia das soluções segue por:
\begin{itemize}
	\item \textbf{\underline{Equação Harmônica/de Laplace}:} \(u_{xx}+u_{yy}=0\)
	\item \textbf{\underline{Equação de Poisson}:} \(u_{xx}+u_{yy}=f \)
\end{itemize}

\begin{figure}[H]
	\begin{center}
		\includegraphics[height=0.4\textheight, width=0.4\textwidth, keepaspectratio]{./Images/omega_05.png}
	\end{center}
\end{figure}
