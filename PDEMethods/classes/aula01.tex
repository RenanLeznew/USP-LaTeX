...
\[
	u'(x)\approx D_{0}u(\overline{x})\coloneqq \frac{u(\overline{x}+h)-u(\overline{x}-h)}{2h} = \frac{1}{2}[D_{+}u(\overline{x}) + D_{-}u(\overline{x})].
\]
...
\[
	u'(\overline{x})\approx D_{3}u(\overline{x})\coloneqq \frac{1}{6h}[2u(\overline{x}+h)+3u(\overline{x})-6u(\overline{x}-h)-u(\overline{x}-2h)],
\]
...
Nós vimos como essas aproximações se comportam, suas importâncias e algumas delas; porém, dado um certo conjunto de pontos onde queremos aproximar a derivada, gostaríamos de determinar qual é a aproximação.
...
\begin{minted}[linenos=true, bgcolor=DarkBackground]{matlab}
