...
\begin{proof*}
	...
	\[
		\sum\limits_{i, j}^{}a_{ij}\alpha_{i}\beta_{j}= \sum\limits_{i=1}^{n}\biggl(\sum\limits_{j=1}^{m}a_{ij}\beta_{j}\biggr)\alpha_{i},
	\]
...
\[
	\underbrace{F\subseteq F(\alpha )}_{(I)}\stackrel{(II)}\subseteq F(\alpha )(\beta )=F(\alpha , \beta )\subseteq K;
\]
